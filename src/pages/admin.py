...
def render_admin_panel(brands: list[str], settings: dict, admin_auth: bool) -> html.Div:
    ...
    if not admin_auth:
        return html.Div(
            [
                html.H3("Painel Admin"),
                html.P("Acesso restrito. Informe a senha para liberar as configurações."),
                dcc.Input(id="admin-password", type="password", placeholder="Senha"),
                html.Button("Entrar", id="btn-admin-login", className="primary-btn"),
                html.Div(id="admin-status", className="status-line"),
                html.Div(id="settings-status", className="status-line", style={"display": "none"}),
                html.Div(
                    [
                        dcc.Upload(id="upload-data", className="upload", multiple=False),
                        html.Button("Recarregar dados", id="btn-reload", className="primary-btn"),
                        dcc.Upload(id="upload-logo", className="upload", multiple=False),
                    ],
                    style={"display": "none"},
                ),
            ],
