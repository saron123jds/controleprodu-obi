...
from src.pages import render_dashboard
...
app.layout = html.Div(
    [
        ...
        dcc.Interval(id="init-settings", n_intervals=0, max_intervals=1),
        html.Div(
            [
                sidebar(),
                html.Div(
...
def init_or_reload(_, contents, filename):
    if contents and filename:
        try:
            ...
            if not ok:
                missing_str = ", ".join(missing)
                return no_update, no_update, f"Erro: colunas faltando {missing_str}"
            df = coerce_types(df)
            df = add_derived(df)
            ...
            return df.to_json(date_format="iso", orient="split"), meta, f"Carregado: {filename}"
        except Exception as exc:
            return no_update, no_update, f"Erro ao carregar: {exc}"

    try:
        df, status = load_data()
        meta = {"source": status}
        return df.to_json(date_format="iso", orient="split"), meta, status
    except Exception as exc:
        return pd.DataFrame().to_json(date_format="iso", orient="split"), {"source": "erro"}, f"Erro: {exc}"
...
def render_page(data_json, settings, admin_auth, f_marca, f_colecao, f_processo, f_status_prod, f_status_venc):
    ...
    page = render_dashboard(df, settings, bool(admin_auth))
...
@app.callback(
    ...
    Input("init-settings", "n_intervals"),
...
def save_settings(_, __, upload_contents, upload_filename, weekly_target, workdays, daily_target_override, selected_brands, critical_delay_days):
    triggered_id = ctx.triggered_id

    if triggered_id == "init-settings":
        settings = get_all_settings(DB_PATH)
        return settings, ""

    if triggered_id == "upload-logo" and upload_contents and upload_filename:
    ...
    if triggered_id == "btn-save-settings":
